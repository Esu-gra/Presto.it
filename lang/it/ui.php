<?php
return [
// !NAVBAR
"Categorie"=>"Categorie",
"Inserisci annuncio"=>"Inserisci annnuncio",
"Zona revisore"=>"Zona revisore",
"Messaggi non letti"=>"Messaggi non letti",
"Tutti gli articoli"=>"Tutti gli articoli",
"Cosa stai cercando?"=>"Cosa stai cercando?",
"Lavora con noi"=>"Lavora con noi",
"Cerca"=>"Cerca",
"Benvenuto"=>"Benvenuto",
"Ciao registrati o accedi!"=>"Ciao registrati o accedi!",
"Registrati"=>"Registrati",

//!LOGIN.REGISTER
"Bentornato"=>"Bentornato, accedi al tuo account!",
"Accedi"=>"Accedi",

//!REGISTER
"Registrati!"=>"Registrati!",
"Nome"=>"Nome",
"Conferma password"=>"Conferma password",
"Registrati"=>"Registrati",

//!HOMEPGAE
'welcome'=> 'Benvenuto su Presto.it',
"Dall'essenziale all'eccezionale: trova tutto qui"=>"Dall'essenziale all'eccezionale: trova tutto qui",
"SPEDIZIONE GRATUITA"=>"SPEDIZIONE GRATUITA",
"QUALITÀ GRATANTITA"=>"QUALITÀ GARANTITA",
"OFFERTE GIORNALIERE"=>"OFFERTE GIORNALIERE",
"100% PAGAMENTI SICURI"=>"100% PAGAMENTI SICURI",
'Gli Ultimi Articoli'=>'Gli Ultimi Articoli',
"Dettagli"=>"Dettagli",

//!INDEX
"Articoli"=>"Tutti i nostri articoli!",
"Vuoi saperne di piú?"=>"Vuoi saperne di piú?",
"Nessun articolo trovato"=>"Nessun articolo trovato",

//!SHOW
"Dettaglio"=>"Dettaglio ",
"Nome"=>"Nome:",
"Descrizione"=>"Descrizione:",
"Categoria"=>"Categoria:",
"Torna indietro"=>"Torna indietro",
"Lascia una recensione"=>"Lascia una recensione",
"Recensioni"=>"Recensioni",
"Scritto da"=>"Scritto da:",
"Commento"=>"Commento",
"Mi dispiace, non ci sono recensioni"=>"Mi dispiace, non ci sono recensioni",


//!FOOTER
"Torna alla Home"=>"Torna alla Home",

//!Dettagli
"Dettagli"=>"Dettagli",
];