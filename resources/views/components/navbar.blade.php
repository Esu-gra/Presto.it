    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
                    ...
                    <ul class="dropdown-menu">
                        @foreach ($categories as $category)
                            <li><a class="dropdown-item"
                                    href="{{ route('categoryShow', $category) }}">{{ $category->name }}</a></li>
                        @endforeach
                    ...
                    @if (Auth::user()->is_revisor)
                        <li class="nav-item me-4">
                            <a class="nav-link btn btn-outline-success btn-sm position-relative" aria-current="page"
                                ...
                                Zona Revisore
                                <span
                                    class="position-absolute top-0 start-100 transale-middle badge rounded-pill bg-danger">
            ...
            <form action="{{ route('articles.search') }}" method="GET" class="d-flex">
                <input name="searched" type="text" class="form-control me-2" placeholder="Cosa stai cercando?"
                    aria-label="Search">
